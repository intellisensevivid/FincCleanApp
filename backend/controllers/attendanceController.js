...
const getAllAttendance = async (req, res) => {
  ...
  try {
    const allAttendance = await Attendance.find({business:req.user.business});
...
const updateAttendance = async (req, res) => {
  try {
    ...
    const attendance = await Attendance.findById({_id:attendanceId,business:req.user.business});
    if (!attendance) {
      return res.status(404).json({ message: 'Attendance not found.' });
    }
    if (clockIn) {
